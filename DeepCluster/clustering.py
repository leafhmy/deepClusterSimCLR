...
def make_graph(xb, nnn, use_gpu=False):
...
class Kmeans(object):
    ...
    def cluster(self, data, verbose=False, use_gpu=False):
        ...
        # cluster the data
        I, loss = run_kmeans(xb, self.k, verbose, use_gpu=use_gpu)
        self.images_lists = [[] for i in range(self.k)]
        for i in range(len(data)):
            self.images_lists[I[i]].append(i)
            # image_lists[weibiaoqian] [yuanshisuoyin, ...]
...
class PIC(object):
    ...
    def cluster(self, data, verbose=False):
        ...
        # construct nnn graph
        I, D = make_graph(xb, self.nnn, use_gpu=self.use_gpu)
